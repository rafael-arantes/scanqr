...
export default function UpgradePage() {
  ...
  useEffect(() => {
    const fetchUserTier = async () => {
      ...
      if (!session) {
        // Usuário não está logado, mas pode ver os planos
        setIsLoggedIn(false);
        return;
      }
      ...
      // Verificar se há um plano pendente após login
      const pendingPlan = localStorage.getItem('pendingUpgrade');
      if (pendingPlan && pendingPlan === 'pro') {
        localStorage.removeItem('pendingUpgrade');
        // Pequeno delay para garantir que o componente renderizou
        setTimeout(() => {
          const upgradeButton = document.querySelector('[data-upgrade-pro]') as HTMLButtonElement;
          if (upgradeButton) {
            upgradeButton.click();
          }
        }, 500);
  ...
  const handlePlanClick = (tier: SubscriptionTier) => {
    if (!isLoggedIn) {
      // Salvar intenção de upgrade no localStorage
      if (tier === 'pro') {
        localStorage.setItem('pendingUpgrade', tier);
      }
      router.push('/login');
    }
  };
  ...
  const handleUpgrade = (tier: SubscriptionTier) => {
    ...
    if (tier === 'enterprise') {
      ...
      window.location.href = 'mailto:vendas@vertio.com.br?subject=Interesse no Plano Enterprise do ScanQR';
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      {/* Header */}
      <nav className="border-b bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm sticky top-0 z-10">
          ...
          {plans.map((plan) => {
            ...
            return (
              ...
              >
                ...
                {/* CTA Button */}
                {!isLoggedIn ? (
                  // Usuário não logado - mostrar botão de login para todos os planos
                  <Button
                    ...
                    size="lg"
                  >
                    {plan.tier === 'free' ? 'Criar conta grátis' : plan.cta}
                  </Button>
                  ...
                  <UpgradeButton tier="pro" className="w-full" variant={plan.popular ? 'default' : 'outline'} data-upgrade-pro>
                    {plan.cta}
                  </UpgradeButton>
                ) : null}
              </div>
      ...
      <section className="container mx-auto px-4 py-16 border-t">
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400">
              Seus QR Codes param de funcionar temporariamente até você fazer upgrade do plano ou até o próximo ciclo mensal.
              Faça upgrade para garantir que seus links continuem sempre ativos.
            </p>
          </div>
          <div>
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50 mb-2">
