...
export default async function ShortIdPage({ params }: { params: Promise<{ shortId: string }> }) {
  ...
  // Note: Server-side event tracking for QR scans removed because:
  // 1. Redirects happen immediately, preventing tracking
  // 2. Umami API requires authentication tokens not suitable for server-side
  // 3. The scan_count in database already tracks this metric
  // Alternative: Track client-side on the destination page if needed
